...
ORDER BY CreatedAt DESC", role, userName);
        ...
        public static OrderRequestDto CreateCustomerToSeller(CreateCustomerOrderDto request)
        {
            ...
            NotificationRepository.Add("Seller", request.RequestedToUser, $"Customer {request.RequestedByUser} ordered {request.Quantity} x {request.BlanketName} ({request.Sku}) from seller {request.RequestedToUser}.", "CustomerOrder", created.Id);
            return created;
        }
        ...
        public static OrderRequestDto CreateSellerToDistributor(CreateSellerRequestDto request)
        {
            ...
            var created = Insert(dto);
            NotificationRepository.Add("Distributor", distributorUserName, $"Seller {request.RequestedByUser} requested {request.Quantity} x {request.BlanketName} ({request.Sku}).", "OrderRequest", created.Id);
            return created;
        }
        ...
        public static OrderRequestDto EscalateToManufacturer(int sellerRequestId, RequestActionDto action)
        {
            ...
            NotificationRepository.Add("Manufacturer", AuthService.GetApprovedUserNameByRole("Manufacturer"), $"Distributor {action.PerformedByUser} escalated request #{source.Id} for {source.Sku}.", "Escalation", created.Id);
            NotificationRepository.Add("Seller", source.RequestedByUser, $"Your request #{source.Id} was escalated to manufacturer.", "Escalation", source.Id);
            return created;
        }
        ...
        public static bool MarkDistributorFulfilled(int requestId, RequestActionDto action)
        {
            ...
            NotificationRepository.Add("Seller", request.RequestedByUser, $"Request #{requestId} was fulfilled by distributor and added to seller inventory.", "Fulfillment", requestId);
            return true;
        }
        ...
        public static bool MarkManufacturerProductionStarted(int requestId, RequestActionDto action)
        {
            ...
            if (ok)
            {
                NotificationRepository.Add("Distributor", request?.RequestedByUser, $"Manufacturer started production for request #{requestId}.", "Production", requestId);
            }
            return ok;
        }
        ...
        public static bool MarkManufacturerDispatched(int requestId, RequestActionDto action)
        {
            ...
            NotificationRepository.Add("Distributor", distributorUserName, $"Request #{requestId} dispatched and added to distributor inventory.", "Dispatch", requestId);
            return true;
        }
            ...
            using (var connection = new SqlConnection(ConnectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    if (hasStock)
                    {
                        ...
                        transaction.Commit();
                        NotificationRepository.Add("Customer", request.RequestedByUser, $"Your order #{requestId} was confirmed by seller.", "CustomerOrder", requestId);
                        return true;
                    }
                    ...
                    transaction.Commit();
                    NotificationRepository.Add("Distributor", sellerRequest.RequestedToUser, $"Seller created request #{sellerRequestId} for customer order #{requestId} ({request.Sku}).", "OrderRequest", sellerRequestId);
                    NotificationRepository.Add("Customer", request.RequestedByUser, $"Your order #{requestId} was forwarded to distributor via seller due to stock shortage.", "CustomerOrder", requestId);
                    return true;
                }
            }
        }
        ...
        public static bool CancelBySeller(int requestId, RequestActionDto action)
        {
            ...
            if (ok)
            {
                NotificationRepository.Add("Distributor", request?.RequestedToUser, $"Seller cancelled request #{requestId}.", "Cancellation", requestId);
            }
            return ok;
        }
        ...
        public static bool CancelByDistributor(int requestId, RequestActionDto action)
        {
            ...
            if (ok)
            {
                NotificationRepository.Add("Seller", request?.RequestedByUser, $"Distributor cancelled request #{requestId}.", "Cancellation", requestId);
            }
            return ok;
        }
        ...
        public static bool CancelByManufacturer(int requestId, RequestActionDto action)
        {
            ...
            if (ok)
            {
                NotificationRepository.Add("Distributor", request?.RequestedByUser, $"Manufacturer cancelled request #{requestId}.", "Cancellation", requestId);
            }
            return ok;
        }
